"""Robust official testing procedures that handle rate limits gracefully"""
import requests
import json
...
def test_solve_task_endpoint(base_url: str = "http://localhost:8080"):
    """Test solve_task endpoint with official format (handles rate limits)"""
    try:
        ...
        # Try with shorter timeout first (for template agent)
        try:
            response = requests.post(
                f"{base_url}/solve_task",
                json=task,
                headers={"Content-Type": "application/json"},
                timeout=10  # Short timeout for template agent
            )
        except requests.exceptions.Timeout:
            # If timeout, try with longer timeout (for LLM with rate limits)
            print("   ⚠️  Initial request timed out, retrying with longer timeout...")
            response = requests.post(
                f"{base_url}/solve_task",
                json=task,
                headers={"Content-Type": "application/json"},
                timeout=90  # Longer timeout for rate-limited LLM
            )
...
def test_response_time(base_url: str = "http://localhost:8080"):
    """Test response time (handles rate limits gracefully)"""
    try:
        task = {
            "id": "test-speed",
        ...
        # For rate-limited scenarios, allow longer response times
        if elapsed > 2.0:
            print(f"⚠️  Response time: SLOW ({elapsed:.2f}s) - likely rate-limited")
            print("   This is acceptable when LLM is rate-limited.")
        else:
...
def test_action_format(base_url: str = "http://localhost:8080"):
    """Test action format compliance with IWA BaseAction (handles rate limits)"""
    try:
        task = {
            "id": "test-format",
...
def run_official_tests(base_url: str = "http://localhost:8080"):
    """Run all official tests with rate limit handling"""
    print("=" * 60)
    print("OFFICIAL IWA MINER TESTING (Rate-Limit Aware)")
    print("=" * 60)
    print()
    ...
    if passed == total:
        ...
        print()
        print("Note: Timeouts due to rate limits are acceptable.")
        print("The API automatically falls back to template agent.")
    else:
